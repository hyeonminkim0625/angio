...
class Consistency_model_wrapper(nn.Module):
    """Some Information about Consistency_model_wrapper"""
...
class BaseLine_wrapper(nn.Module):
    ...
    def __init__(self,args):
        ...
        if self._model == "deeplab":
            ...
            self.model = UNet(3,_num_classes)
...
class Conv3D_wapper(nn.Module):
    """Some Information about Conv3D_wapper"""
    def __init__(self,args):
        super(Conv3D_wapper, self).__init__()
        _num_classes = args.num_classes
        self.model = UNet_3D(3,_num_classes)
    ...
    def forward(self, x):
        ...
        for support in x['support']:
            temp.append(support)
        temp.append(x['anchor'])
        return self.model(torch.stack(temp,dim = 2))['out']
...
class UNet_3D(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=1, init_features=32):
        ...
        #[b, c, seq, h, w]
        self.encoder1 = UNet_3D._block(in_channels, features, name="enc1")
        self.pool1 = nn.MaxPool3d(kernel_size=(1,2,2), stride=(1,2,2))
        self.encoder2 = UNet_3D._block(features, features * 2, name="enc2")
        self.pool2 = nn.MaxPool3d(kernel_size=(1,2,2), stride=(1,2,2))
        self.encoder3 = UNet_3D._block(features * 2, features * 4, name="enc3")
        self.pool3 = nn.MaxPool3d(kernel_size=(1,2,2), stride=(1,2,2))
        self.encoder4 = UNet_3D._block(features * 4, features * 8, name="enc4")
        self.pool4 = nn.MaxPool3d(kernel_size=(1,2,2), stride=(1,2,2))

        self.bottleneck = UNet_3D._block(features * 8, features * 16, name="bottleneck")

        self.upconv4 = nn.ConvTranspose3d(
            features * 16, features * 8, kernel_size=(1,2,2),stride=(1,2,2)
        )
        self.decoder4 = UNet_3D._block((features * 8) * 2, features * 8, name="dec4")
        self.upconv3 = nn.ConvTranspose3d(
            features * 8, features * 4, kernel_size=(1,2,2),stride=(1,2,2)
        )
        self.decoder3 = UNet_3D._block((features * 4) * 2, features * 4, name="dec3")
        self.upconv2 = nn.ConvTranspose3d(
            features * 4, features * 2, kernel_size=(1,2,2),stride=(1,2,2)
        )
        self.decoder2 = UNet_3D._block((features * 2) * 2, features * 2, name="dec2")
        self.upconv1 = nn.ConvTranspose3d(
            features * 2, features, kernel_size=(1,2,2),stride=(1,2,2)
        )
        self.decoder1 = UNet_3D._block(features * 2, features, name="dec1")

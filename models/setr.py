...
class SETR(nn.Module):
    def __init__(self, embed_dim = 256, patch_size = 8):
        ...
        self.decoder_upscale = nn.Upsample(scale_factor=2, mode='bilinear')

        self.upscale = nn.Upsample(scale_factor=4, mode='bilinear')

    def forward(self, x):
        #batch channel h w
        x = self.proj(x) + positionalencoding2d(256,64,64).unsqueeze(0).to('cuda')
        x = x.flatten(2,3).permute(2,0,1)
        x = self.transformer_encoder(x)
        x = x.permute(1,2,0).view(-1,256,64,64)
